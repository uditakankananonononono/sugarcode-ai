"""Drop 45/47: exonic donor/acceptor-window sweep. ClinVar cites exonic splice-region
...
SIDE = "donor"  # or "acceptor": acceptor key c.N = first coding base of
# the incoming exon -> window index 14 (the only exonic position in the
# 15-mer acceptor window)
...
def main():
    ...
    for sym in sorted(GENES):
        ...
        for doc in docs.values():
            ...
            if SIDE == "donor":
                ...
                targets = ((N, w, 2 - (N - n)) for N, w in donors.items())
                in_range = lambda i: 0 <= i <= 2
            else:
                # first exonic base after an acceptor: c.N -> index 14
                targets = ((N, w, 14 if N == n else -1)
                           for N, w in acceptors.items())
                in_range = lambda i: i == 14
            for N, w, idx in targets:
                if in_range(idx) and w[idx] == refb:
                    ...
                    res = variant_at(w, idx, altb, SIDE)
                    off = (idx - 3) if SIDE == "donor" else (idx - 14 + 1)
                    cases.append({"notation": f"c.{n}{refb}>{altb}",
                                  "exonic_offset": off,  # donor: -1 terminal; acceptor: +1 first base
                                  "site_type": SIDE, "window": w,
    ...
    fnames = {"donor": "exonic_donor_golden.json",
              "acceptor": "exonic_acceptor_golden.json"}
    json.dump({"source": f"ClinVar live unfiltered sweep 2026-09-22: variants cited as "
                         f"coding substitutions (c.NX>Y) at exon-boundary {SIDE} "
                         f"positions, bucketed on germline description text",
               "side": SIDE,
               "genes": out, "summary": summary},
              open(f"tests/fixtures/{fnames[SIDE]}", "w"), indent=1)
