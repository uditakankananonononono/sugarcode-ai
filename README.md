...
pip install .
sugarcode version
sugarcode modules                       # the 91 registered modules (77 spec + 14 beyond-spec)
...
src/omega/            Omega OS v7.0 framework
  registry.py         All 91 registered modules (77 spec + 14 beyond-spec) + 9 sub-networks + lifecycle status
...
Counts include the 14 beyond-spec modules registered after the audit fix
...
Beyond the spec, 14 more real published-model/computational modules were built
(cfd_offtarget, crisprater, crisprscan_score, mit_offtarget, structural_biophysics,
syn_bio_studio, rna_nussinov, dti_bench, acmg_bayesian, pgx_guidelines, evidence_mining, neuro_hub_dashboard,
qsar_bench, molecule_eval). Since the audit-fix drop every one of them is registered,
so the canonical registry now holds **91 modules**, every registry slug equals its
...
| rna_nussinov (current, pb6) | **91 registered (77 spec + 14 beyond-spec)**; Nussinov-Jacobson 1980 max base-pair RNA folding (traceback, min loop, dot-bracket, exact optimal-structure count, stats), hand fixtures + brute-force verifier | 0 | 0 | 1426 passing, 1 skipped |
