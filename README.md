...
pip install .
sugarcode version
sugarcode modules                       # the 94 registered modules (77 spec + 17 beyond-spec)
...
src/omega/            Omega OS v7.0 framework
  registry.py         All 94 registered modules (77 spec + 17 beyond-spec) + 9 sub-networks + lifecycle status
...
Beyond the spec, 17 more real published-model/computational modules were built
...
qsar_bench, molecule_eval, chem_descriptors, chem_similarity). Since the audit-fix drop every one of them is registered,
so the canonical registry now holds **94 modules**, every registry slug equals its
