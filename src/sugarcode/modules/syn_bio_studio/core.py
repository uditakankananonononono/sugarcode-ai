...
def compile_circuit(behavior,inputs=(),parts=None):
 p=parts or {}; architectures={'NOT':['input','repressor','output'],'AND':['input_a','input_b','coactivator','output'],'toggle':['repressor_A','repressor_B'],'oscillator':['repressor_A','repressor_B','repressor_C'],'sensor':['ligand_receptor','promoter','output']}; key=behavior if behavior in architectures else 'sensor'; return {'behavior':behavior,'nodes':architectures[key],'parts':p,'edges':[{'source':a,'target':b,'relation':'regulates'} for a,b in zip(architectures[key],architectures[key][1:])],'inputs':list(inputs),'status':'architecture-level, non-procedural'}
...
def simulate_oscillator(hours=48,initial=(.1,.2,.3),alpha=2,beta=.5):
 ...
 t=np.linspace(0,hours,481); s=solve_ivp(rhs,(0,hours),initial,t_eval=t,rtol=1e-8,atol=1e-9); peaks=sum(s.y[0,i]>s.y[0,i-1] and s.y[0,i]>s.y[0,i+1] for i in range(1,len(t)-1)); return {'time_h':t.tolist(),'species':s.y.tolist(),'peak_count':peaks,'amplitude':float(np.ptp(s.y[0]))}
