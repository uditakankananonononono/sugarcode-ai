...
def compute_flux_series(samples):
 return {'timestamps':[x['timestamp'] for x in samples],'global_flux':[x['modules_online']/max(1,x['modules_total']) for x in samples],'trend':(samples[-1]['modules_online']/samples[-1]['modules_total']-samples[0]['modules_online']/samples[0]['modules_total']) if samples else 0}
