...
_CACHE = Path(os.getenv("SUGARCODE_CLINICAL_CACHE", Path.home() / ".sugarcode_cache" / "acmg_bayesian"))
_LOCK = threading.Lock()
_LAST_REQUEST = 0.0


_DEL_DUP_BASES = re.compile(r"(del|dup)[ACGTacgt]+$")


def normalize_variant_notation(hgvs: str) -> str:
    """Variant part of an HGVS string in ClinVar's current style.

    Drops the reference/transcript prefix and the redundant deleted/duplicated
    bases (HGVS recommends ``c.68_69del`` over ``c.68_69delAG``; ClinVar titles
    use the short form). ``delins`` is left untouched.
    """
    needle = hgvs.split(":")[-1].strip().replace(" ", "")
    return _DEL_DUP_BASES.sub(lambda m: m.group(1), needle)


def title_matches(title: str, needle: str) -> bool:
    """True when ``needle`` appears in a ClinVar title as a whole variant token
    (so ``c.68_69del`` does not match ``c.68_69delinsT`` or ``c.68_69dup``)."""
    return re.search(re.escape(needle) + r"(?![A-Za-z0-9_>])", title) is not None
...
class NCBIClient:
    ...
    def __init__(self, cache_dir: str | Path | None = None, *, api_key: str | None = None,
                 tool: str = "sugarcode-acmg-bayesian",
    ...
    def clinvar(self, gene: str, hgvs: str, *, retmax: int = 20, offline: bool = False) -> dict:
        needle = normalize_variant_notation(hgvs)
        ...
        for uid in result.get("uids", []):
            ...
            records.append({"variation_id": uid, "title": title,
                            "exact_notation_in_title": title_matches(title, needle),
        ...
        return {"query": term, "normalized_notation": needle, "records": records, "search_meta": search_meta,
