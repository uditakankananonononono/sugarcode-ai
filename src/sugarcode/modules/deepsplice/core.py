...
def exon_inclusion(ref_site,alt_site,regulatory,calibration_n=100):
    delta=alt_site-ref_site; logit_ref=-.5+5*ref_site+2*regulatory['exon_definition_support']; logit=logit_ref+delta*max(5,logit_ref/.15)  # anchored to ref; for a strong ref site the calibrated -0.15 loss band (variant_effect) lands at psi=0.5
    psi=1/(1+math.exp(-logit)); radius=1.96*math.sqrt(psi*(1-psi)/max(1,calibration_n)); return {'psi':psi,'delta_site_strength':delta,'interval95':[max(0,psi-radius),min(1,psi+radius)],'calibration_n':calibration_n}
