...
def _cds_to_locus(c_pos,seq):
    """Map an HGVS c. coordinate (A of ATG = c.1, c.-1 = base before it) onto the 1-based
    locus sequence using the longest + strand ORF as the CDS. Before this, c. numbers were
    indexed straight into the transcript (ignoring the 5'UTR), and a full ClinVar name like
    'NM_000518.5(HBB):c.20A>T' was parsed as position 518 from the accession digits."""
    cands=[o for o in orfs(seq,min_aa=30,both_strands=False)]
    if not cands: return c_pos
    start=max(cands,key=lambda o:o["aa_length"])["start"]
    return start+c_pos if c_pos>0 else start+c_pos+1

# Variant-connected v2 overrides.
def _variant_features(variant,seq=''):
    """Derive variant and locus-context features; user-supplied annotations win."""
    variant=variant or {}; seq=clean_dna(seq) if seq else ''; consequence=str(variant.get('consequence','')).lower(); coordinate=str(variant.get('variant','')); import re; cmatch=re.search(r'c\.(-?\d+)',coordinate); match=cmatch or re.search(r'(\d+)',coordinate); position=int(variant.get('position',match.group(1) if match else max(1,len(seq)//2))); mapped=bool(cmatch and 'position' not in variant and seq and int(cmatch.group(1))>0); position=_cds_to_locus(position,seq) if (cmatch and 'position' not in variant and seq) else position; cds0=(position-int(cmatch.group(1))) if mapped else 0; zero=max(0,min(len(seq)-1,position-1)) if seq else 0; local=seq[max(0,zero-6):min(len(seq),zero+7)]; codon_start=cds0+((zero-cds0)//3)*3; codon=seq[codon_start:codon_start+3] if seq else ''; local_gc=(local.count('G')+local.count('C'))/max(1,len(local)); position_fraction=position/max(1,len(seq)); splice=splice_outcome(consequence,position,max(60,len(seq)//3)); synonymous='synonymous' in consequence; stop=any(x in consequence for x in ('nonsense','stop','frameshift')); missense='missense' in consequence; domain_overlap=bool(variant.get('domain_overlap',.2<=position_fraction<=.8)); context_constraint=.35*local_gc+.25*domain_overlap+.2*(codon in {'TGG','ATG'})+.2*(1-abs(.5-position_fraction)*2)
    conservation=float(variant.get('conservation',max(0,min(1,(.1 if synonymous else .9 if stop else .55 if missense else .4)+.25*(context_constraint-.5))))); regulatory=float(variant.get('regulatory',max(0,min(1,(.05 if synonymous else .5 if 'regulatory' in consequence else .12)+.25*local_gc)))); splicing=float(variant.get('splicing',splice['splice_disruption'])); interface=bool(variant.get('interface',(missense or stop) and domain_overlap)); active=bool(variant.get('active_site',stop or (missense and codon in {'ATG','TGG'}))); functional=float(variant.get('functional_score',max(0,min(1,(.05 if synonymous else .9 if stop else .6 if missense else .4)+.2*context_constraint))))
    return {"consequence":consequence,"position":position,"position_fraction":position_fraction,"local_context":local,"local_gc":local_gc,"codon":codon,"domain_overlap":domain_overlap,"context_constraint":context_constraint,"synonymous":synonymous,"stop_or_frameshift":stop,"conservation":conservation,"regulatory":regulatory,"splicing":splicing,"interface":interface,"active_site":active,"functional_prior":functional,"splice_outcome":splice}
