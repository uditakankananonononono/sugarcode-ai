...
def enhancement_features(optimization: dict, simulation: dict) -> dict:
    ...
    out={"subsets_evaluated":optimization["evaluated_subsets"],"selected_factor_count":sel["factor_count"],"selected_score":sel["score"],"selected_target_activation":sel["target_activation"],"selected_source_silencing":sel["source_silencing"],"selected_burden":sel["burden"],"score_min":float(scores.min()),"score_max":float(scores.max()),"score_range":float(np.ptp(scores)),"score_margin":float(scores[0]-scores[1]) if len(scores)>1 else 0.0,"activation_min":float(act.min()),"activation_max":float(act.max()),"activation_range":float(np.ptp(act)),"silencing_min":float(sil.min()),"silencing_max":float(sil.max()),"silencing_range":float(np.ptp(sil)),"burden_min":float(burden.min()),"burden_max":float(burden.max()),"duration_h":float(t[-1]),"timepoint_count":len(t),"initial_source_fraction":float(S[0]),"terminal_source_fraction":float(S[-1]),"source_fraction_change":float(S[-1]-S[0]),"source_half_loss_hour":float(t[np.argmax(S<=.5)]) if np.any(S<=.5) else float(t[-1]),"source_auc":float(np.trapz(S,t)),"initial_transition_fraction":float(T[0]),"terminal_transition_fraction":float(T[-1]),"peak_transition_fraction":float(T.max()),"peak_transition_hour":float(t[T.argmax()]),"transition_auc":float(np.trapz(T,t)),"initial_target_fraction":float(F[0]),"terminal_target_fraction":float(F[-1]),"target_fraction_change":float(F[-1]-F[0]),"peak_target_fraction":float(F.max()),"peak_target_hour":float(t[F.argmax()]),"target_auc":float(np.trapz(F,t)),"target_ten_percent_hour":float(t[np.argmax(F>=.1)]) if np.any(F>=.1) else float(t[-1]),"target_half_max_hour":float(t[np.argmax(F>=F.max()/2)]),"initial_stress":float(X[0]),"terminal_stress":float(X[-1]),"peak_stress":float(X.max()),"peak_stress_hour":float(t[X.argmax()]),"stress_auc":float(np.trapz(X,t)),"conversion_efficiency":float(F[-1]/max(1-S[-1],1e-12)),"target_to_stress_ratio":float(F[-1]/max(X[-1],1e-12)),"source_target_crossing_hour":float(t[np.argmax(F>=S)]) if np.any(F>=S) else float(t[-1]),"solver_evaluations":simulation["solver"]["nfev"],"trajectory_mass_terminal":float(S[-1]+T[-1]+F[-1]),"maturation_fraction":float(F[-1]/max(F[-1]+T[-1],1e-12)),"residual_source_burden":float(S[-1]*(1+X[-1]))}
    assert len(out)==50
    return out
