# SugarCode AI - honest status (as of drop 26 / this push)
...
Test suite: **351 passed, 0 failed** (hermetic fixtures; live calls verified
...
| deepsplice multi-gene golden | 17 genes (+ MSH6, PMS2, APC, ATM, RB1, PTEN, PAH, LDLR, SCN1A, MECP2): pooled 2,050 pathogenic + 63 benign ClinVar splice SNVs on title-verified RefSeqGene junction maps. ALL 1,802 canonical U2 (GT/GC-AG) +/-1/+/-2 sites called loss (100%); benign specificity 62/63 (98%, the one FP is the documented BRCA1 c.594-2A>C). Transcript overrides verified live: APC NM_000038 (record span-matches NM_001127511.1), MECP2 NM_001110792; SCN1A ClinVar cites NM_001165963 whose 33-nt alt coding exon is absent from the record - downstream coordinates remapped -33, every case ref-base verified (42 -> 97 mapped, zero mismatches; alt-exon-internal variants stay dropped) | NCBI ClinVar + RefSeqGene x17 |
...
| deepsplice AT-AC site-class detection | Extended golden caught 8 pathogenic canonical variants at two SCN1A AT-AC (U12 minor-spliceosome) introns silently scoring delta=0 'minimal effect' - or +0.25 'strengthened' - under the GT-AG matrix. Now site_class/pwm_applicable gate every score: variant_effect flags them not-interpretable and live_splice_assessment returns status atypical_site_class with no score. AT-AC PWM matrix: Missing (2 observed introns, too few; no honest approximation vendored). Legacy consensus-seed acceptor windows also predate the harvest -2/-1 convention (AG at idx 11/12 vs 12/13); fallback-only, real matrices unaffected | SCN1A golden, RefSeqGene NG_011906.1 |
...
- AT-AC (U12 minor-spliceosome) donor/acceptor PWM: only 2 AT-AC introns
  observed in the harvest (SCN1A c.383/c.4443) - too few for a learned matrix
  and no honest swap approximation exists. AT-AC sites are DETECTED and labeled
  out-of-scope (drop 26), never scored with the GT-AG matrix.
