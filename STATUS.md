# SugarCode AI - honest status (as of drop 24 / this push)
...
Test suite: **318 passed, 0 failed** (hermetic fixtures; live calls verified
...
| deepsplice multi-gene golden | Calibration is not BRCA1-specific: pooled 610 pathogenic + 32 benign ClinVar splice SNVs across BRCA1/BRCA2/MLH1/CFTR (title-verified RefSeqGene junction maps); 517/519 canonical +/-1/+/-2 sites called loss (99.6%); specificity 29/32 (91%). The two pooled canonical misses are one GC-donor site (BRCA2 c.7976+2C>G/A): GC donors were excluded from PWM training by design - scope limit locked in a test, not tuned away | NCBI ClinVar + RefSeqGene x4 |
